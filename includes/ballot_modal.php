    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
              <?php
                $id = $voter['id'];
                $sql = "SELECT *, candidates.cname AS canname FROM votes LEFT JOIN candidates ON candidates.id=votes.candidate_id LEFT JOIN positions ON positions.id=votes.position_id WHERE matricno = '$id'  ORDER BY positions.priority ASC";
                $query = $conn->query($sql);
                while($row = $query->fetch_assoc()){
                  echo "
                    <div class='row votelist'>
                      <span class='col-sm-4'><span class='pull-right'><b>".$row['description']." :</b></span></span> 
                      <span class='col-sm-8'>".$row['canname']."</span>
                    </div>
                  ";
